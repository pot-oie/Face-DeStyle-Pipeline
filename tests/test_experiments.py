...
def test_repository_experiment_matrix_has_primary_and_extensions():
    ...
    assert by_name["global_canny"].settings["controlnet_conditioning_scale"] == 0.4
    assert by_name["region_canny"].settings["face_parsing_model"] == "face_parsing"
    assert by_name["region_canny"].settings["background_edge_scale"] == 0.25
