...
if [[ ${1:-} == "--help" || ${1:-} == "-h" ]]; then
  cat <<'EOF'
Usage: bash scripts/run_origami_lora_v2_holdout_eval.sh

Run the fixed six-source Origami holdout comparison for Base, frozen V1 checkpoint 100,
and V2 checkpoints 50, 100, 150, and 200. Environment variables may override the
default AutoDL paths. Existing complete method outputs are skipped; incomplete methods resume.
EOF
  exit 0
fi
...
V1_TRAIN_OUTPUT=${V1_TRAIN_OUTPUT:-$FACE_DESTYLE_ROOT/outputs/origami-destyle-lora-teacher23-r16-steps300}
V2_TRAIN_OUTPUT=${V2_TRAIN_OUTPUT:-$FACE_DESTYLE_ROOT/outputs/origami-destyle-lora-v2-51-r16-steps200}
RUN_ROOT=${RUN_ROOT:-$FACE_DESTYLE_ROOT/outputs/origami-lora-heldout-v2-base-v1ckpt100-v2ckpt50-100-150-200-seed42}
ARCHIVE=${ARCHIVE:-$FACE_DESTYLE_ROOT/packages/origami-lora-heldout-v2-base-v1ckpt100-v2ckpt50-100-150-200-seed42.zip}
...
on_exit() {
  code=$?
  echo "ORIGAMI_V2_HOLDOUT_EVAL_EXIT_CODE=$code" | tee "$RUN_ROOT/LAST_EXIT.txt"
  if [[ $code -eq 0 ]]; then
...
required_files=(
  ...
  "$MODEL_MANIFEST"
  "$MODEL_HASHES"
  "$V1_TRAIN_OUTPUT/checkpoint-100/pytorch_lora_weights.safetensors"
)
for checkpoint in 50 100 150 200; do
  required_files+=(
    "$V2_TRAIN_OUTPUT/checkpoint-$checkpoint/pytorch_lora_weights.safetensors"
  )
done
...
run_method base
run_method v1-checkpoint-100 "$V1_TRAIN_OUTPUT/checkpoint-100/pytorch_lora_weights.safetensors"
for checkpoint in 50 100 150 200; do
  run_method \
...
{
  echo "schema=origami-lora-heldout-comparison/v2"
  echo "seed=42"
  echo "num_inference_steps=28"
  echo "guidance_scale=2.5"
  echo "lora_scale=1.0"
  echo "holdouts=6"
  echo "methods=base,v1-checkpoint-100,v2-checkpoint-50,v2-checkpoint-100,v2-checkpoint-150,v2-checkpoint-200"
  sha256sum \
    "$V1_TRAIN_OUTPUT/checkpoint-100/pytorch_lora_weights.safetensors" \
    "$V2_TRAIN_OUTPUT/checkpoint-50/pytorch_lora_weights.safetensors" \
    "$V2_TRAIN_OUTPUT/checkpoint-100/pytorch_lora_weights.safetensors" \
    "$V2_TRAIN_OUTPUT/checkpoint-150/pytorch_lora_weights.safetensors" \
    "$V2_TRAIN_OUTPUT/checkpoint-200/pytorch_lora_weights.safetensors"
