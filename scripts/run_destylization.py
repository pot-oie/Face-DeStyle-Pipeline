...
def make_backend(
    ...
    return DiffusersBackend(settings, styles_config)
...
def main() -> int:
    ...
    backend = make_backend(backend_name, config, styles_config)
    if args.metadata:
        if args.records_output is None:
            parser.error("--records-output is required with --metadata")
        records = read_jsonl(
