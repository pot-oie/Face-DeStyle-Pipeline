...
TRAIN_DATA=${TRAIN_DATA:-$FACE_DESTYLE_ROOT/data/origami-lora-pairs-v21-51-templates}
...
OUTPUT_DIR=${OUTPUT_DIR:-$FACE_DESTYLE_ROOT/outputs/origami-destyle-lora-v21-51-templates-r16-steps200}
SCREEN_NAME=${SCREEN_NAME:-origami-lora-v21-51}
...
if [[ -e "$TRAIN_DATA" ]]; then
  python scripts/build_origami_lora_v21_dataset.py --output "$TRAIN_DATA" --verify-only
else
  python scripts/build_origami_lora_v21_dataset.py \
    --source "$SOURCE_DATA" \
    --output "$TRAIN_DATA"
fi
...
screen -L -Logfile "$OUTPUT_DIR/screen.log" -dmS "$SCREEN_NAME" \
  ...
  --instance_prompt "Turn the entire folded-paper origami portrait into a natural realistic photograph while preserving identity and composition." \
